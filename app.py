...
# Page Config
st.set_page_config(
    page_title="Financial Inclusion Analysis Dashboard - India",
    page_icon="",
    layout="wide",
...
with st.sidebar:
    ...
    st.markdown("**India's PMJDY Analysis Monitor**")
    st.markdown("---")
    st.markdown("### Navigate To")
    st.page_link("app.py", label="Home")
    st.page_link("pages/1_National_View.py", label="National View")
    st.page_link("pages/2_State_Analysis.py", label="State Analysis")
    st.page_link("pages/3_District_View.py", label="District Explorer")
...
<div class='gov-header'>
    <h1 style='margin:0; font-size:28px;'>Financial Inclusion Analysis Dashboard - India</h1>
    ...
    <p style='margin:8px 0 0 0; font-size:13px; opacity:0.8;'>
        Real data from Ministry of Finance
    </p>
</div>
...
with st.expander("What is PMJDY? - Click to read", expanded=True):
    ...
    with col1:
        ...
        **What it offers every account holder:**
        ...
        - Rs. 10,000 overdraft facility (for active accounts)
    ...
    with col2:
        ...
        **2015** - 17.5 crore accounts opened in first year

        **2018** - Overdraft limit doubled to Rs. 10,000

        **2020** - COVID relief via DBT to PMJDY accounts

        **2022** - Extended to 2025

        **2024** - 45.8 crore accounts, Rs. 2.31 lakh crore deposits
...
col1.metric("Total PMJDY Accounts", f"{total_accounts/1e7:.1f} Cr", "Across all States & UTs")
col2.metric("Total Deposits", f"Rs.{total_deposit/1e5:.1f} Lakh Cr", "Real money in accounts")
col3.metric("Avg Balance / Account", f"Rs.{avg_balance:.0f}", "National average")
...
with col2:
    st.markdown("""
    <div class='kpi-card'>
        <h4 style='color:#1F4E79; margin:0'>The Activation Gap</h4>
        ...
        <p style='margin:8px 0 0 0; font-size:12px; color:#E74C3C;'>Accounts opened vs accounts used</p>
    </div>
    """, unsafe_allow_html=True)
...
with col3:
    ...
    <div class='kpi-card'>
        ...
        <p style='margin:0; font-size:14px;'>gap between highest avg balance state (Goa: Rs.7,611)
        and lowest (Nagaland: Rs.2,804). Geography shapes financial behaviour.</p>
...
with col1:
    ...
    top5.columns = ["State", "Avg Balance (Rs.)", "Accounts (Lakh)"]
...
with col2:
    ...
    bot5.columns = ["State", "Avg Balance (Rs.)", "Accounts (Lakh)"]
...
with col1:
    st.markdown("""
    **National View**
    See all states ranked, mapped, and compared. Find which regions are leading and which are lagging.
...
with col1:
    ...
    <div style='background:#FFFFFF;border-left:4px solid #2563B0;border-radius:6px;padding:10px 14px;font-size:13px;color:#1E293B;line-height:1.6;'>
        ...
        <b>Policy Researchers</b> - Understand where financial inclusion is working and where it is not<br>
        ...
        <b>Banking Professionals</b> - Identify underserved districts for branch expansion<br>
...
with col2:
    ...
    <div class='warning-box'>
        ...
        State-level data covers all <b>States & UTs</b>.
    </div>
    """, unsafe_allow_html=True)
...
#
st.markdown("---")
st.markdown("<div class='gov-footer'>Financial Inclusion Analysis Dashboard &nbsp;|&nbsp; Independent Research Project &nbsp;|&nbsp; Built by Tejaswini S S Mathpati &nbsp;|&nbsp; 2026</div>", unsafe_allow_html=True)
